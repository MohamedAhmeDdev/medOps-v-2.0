...
function UpdateMedicine() {
   ...
   const navigate = useNavigate()
    ...
    useEffect(() => {
      const getSingCategory = async () => {
        const res = await axios.get(`${LOGISTICS_SERVER_URL}/medicine/${id}`);
  ...
  try {
      ...
      await axios.patch(`${LOGISTICS_SERVER_URL}/medicine/${id}`,  formData, {
        headers: {
                                ...
                                <div className="flex-auto p-6">
                                  ...
                                  <div className="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                      <div className="mb-4 py-3 ">
                                        ...
                                        <input type="file"  accept="image/*"  onChange={(e) => setMedicine_image(e.target.files[0])} className="focus:shadow-lg focus:shadow-gray-100 text-sm ease block w-full rounded-md border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-gray-500 focus:outline-none"/>
                                  ...
                                  <button className="px-8 mt-3 py-3 w-11/12 ml-5 font-bold leading-normal text-center text-white transition-all ease-in bg-gray-500 border-0 rounded-lg shadow-md cursor-pointer text-lg hover:shadow-xs hover:-translate-y-px active:opacity-85">Update</button>
                                </div>
                              </form>
                              </div>
                          </div>
