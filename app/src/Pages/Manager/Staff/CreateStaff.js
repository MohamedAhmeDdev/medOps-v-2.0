...
function CreateStaff() {
  ...
  const create_staff = async (e) => {
    e.preventDefault();
  
    try {
      await axios.post(`${SERVER_URL}/Manager/StaffWarehouse`, {
        username: username,
        email: email,
      ...
      if (error.response?.status === 400) {
        ...
        showErrorNotification('Email already exists.');
      }
    }
  };
           ...
           <div className="max-w-full pb-20 lg:mb-0 lg:w-full lg:flex-none">
                <div className="relative flex flex-col min-w-0 mt-6 break-words bg-white border-0 border-transparent border-solid rounded-lg bg-clip-border">
                  <div className="p-4 mb-4">
                    <h3 className="mb-4 text-xl font-semibold text-black">create Staff information</h3>
                    <form onSubmit={create_staff}>
                      <div className="grid grid-cols-6 gap-6">
                          ...
                          <div className="col-span-6 sm:col-span-3 py-3">
                              ...
                              <input  value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} type="text" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 focus:bg-white focus:outline-none"/>
                          </div>
                          <div className="col-span-6 sm:col-span-3 py-3">
